...
namespace Nuclex.UserInterface.Visuals.Flat {
  ...
  partial class FlatGuiGraphics {
    ...
    /// <summary>Draws a GUI element onto the drawing buffer with a texture</summary>
    /// <param name="frameName">Class of the element to draw</param>
    /// <param name="bounds">Region that will be covered by the drawn element</param>
    /// <param name="texture">Texture image to draw</param>
    public void DrawElement(string frameName, RectangleF bounds, Texture2D texture, Color color)
    {
        var destination = new Rectangle((int) bounds.X, (int) bounds.Y, (int) bounds.Width, (int) bounds.Height);
